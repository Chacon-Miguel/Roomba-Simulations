...
class Room(object):
    ...
    def __init__(self, width, height, dust_amount):
        ...
        self.width = width
        self.height = height
        # dict whose keys are tuples of the coordinates and value is dust amount
        self.tiles = {(x, y): dust_amount for x in range(width) for y in range(height)}
    ...
    def clean_tile_at_position(self, pos, cleaning_volume):
        ...
        """
        # change floats to ints using floor
        x, y = math.floor(pos.get_x()), math.floor(pos.get_y())
        # get dust at position
        dust_at_pos = self.tiles[(x, y)]
        # make sure dust amount is non-negative
        if dust_at_pos <= cleaning_volume:
            self.tiles[(x, y)] = 0
        else:
            self.tiles[(x, y)] -= cleaning_volume
...
class NormalRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # calculate next position of robot
...
class SensingRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        max_dust_angles = {}
        for (angle, amount) in dust_amounts.items():
            if amount == max_dust:
                max_dust_angles[angle] = amount
        angle = random.choice(list(max_dust_angles.keys()))
        new_pos = self.position.get_new_position(angle, self.speed)
        self.set_position(new_pos)
        self.room.clean_tile_at_position(new_pos, self.cleaning_volume)
...
def run_simulation(num_robots, speed, cleaning_volume, width, height, dust_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        robots = [robot_type(room, speed, cleaning_volume) for robot in range(num_robots)]
        time_steps = 0
...
#       of a 20x20 room?
#       The Sensing Robot takes the shortest time for all number of robots.
#       The Clumsy Robot took up the most time for all number of robots
#       The difference between the Normal Robot and the Clumsy Robot is not as drastic
#       as the difference between the Normal Robot and the Sensing Robot
#
# 2) How does the performance of the three robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
#       All the robots did their best when the room was closest to a square (aspect ratio of 1)
#       As the aspect ratio got larger (i.e. the room got wider), the time it took each robot
#       also increased. But between the robots, the Sensing Robot did the best, and the Clumsy Robot
#       did the worst at every aspect ratio.
